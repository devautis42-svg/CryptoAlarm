...
# Extend live payload support with an exact kline update for Futures charts.
...
# Constructor gets selected chart interval plus kline callback.
socket = socket.replace(
...
# Futures: connect directly to the candle stream for the selected timeframe.
...
# Replace message parser so a Futures kline event directly updates the candle.
...
# Add one thread-safe connection-state helper.
...
'''
    if insert_before not in socket:
        raise RuntimeError("Could not locate scheduleReconnect")
    socket = socket.replace(insert_before, helper + insert_before, 1)

# Exact merge of a kline payload into the existing REST-seeded candle list.
...
# Add a separate queue for Futures candle payloads.
...
if "liveKlines" not in app:
    app = app.replace(
        trade_queue,
        trade_queue + '    val liveKlines = remember(market, symbol, timeframe.api) { ConcurrentLinkedQueue<LiveKlineUpdate>() }\n',
        1
    )

# Clear both queues when market/symbol/timeframe changes.
app = app.replace(
...
# Replace 33ms batching loop: Futures consumes exact kline snapshots; Spot still consumes trades.
...
# Pass interval and kline callback to socket.
app = app.replace(
    '''            marketType = market,
